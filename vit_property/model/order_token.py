...
class order_token(models.Model):
	...
	total_amount = fields.Float( string=_("Total Amount"))
	date = fields.Datetime( string=_("Date"))
	status = fields.Selection(selection=[("draft", "Draft"),("reserved", "Reserved"),("confirmed", "Confirmed")], default="draft",  string=_("Status"))
	...
	def create(self, vals):
		for val in vals:
			if not val.get("name", False) or val["name"] == "New":
				val["name"] = self.env["ir.sequence"].next_by_code("vit.order_token") or "Error Number!!!"
	...
	def copy(self, default=None):
		...
		return super(order_token, self).copy(default)
